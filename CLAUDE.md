...
[2026-05-14] [FE4 / RiskAssessmentsExt PageController] Learning: A FE4 extension controller registered via `controllerName` in manifest.json MUST extend `sap.fe.core.PageController` — not return a plain object literal. When a plain object is registered, FE4 calls the press handler methods with `this` = the plain object which has no `getView()` method, so any attempt to call `this.getView()` is undefined. Stored `_oCtrl = this` in the `onOpen*` methods (called with the correct controller `this` by FE4) and changed `_getCtx()` to use `_oCtrl.getView().getBindingContext()`. Also: after `oCtx.setProperty(lFld, selL)` + `setProperty(cFld, selC)`, do a client-side score update immediately (score = selL × selC, level = scoreToLevel(score)) so the form shows the new values before the server recalculates. The try/catch around computed field setProperty is required — server-computed fields are often read-only in V4 and setProperty throws; the server recalculates them correctly on next GET.
Source: User report — "No binding context — cannot save" on Risk Matrix Apply button
Applied: app/admin-bridges/webapp/ext/controller/RiskAssessmentsExt.js — PageController.extend + _oCtrl pattern
...
[2026-05-22] [BTP / html5-app-deployer root cause] Learning: "Failed to load UI5 component for navigation intent '#Bridges-manage'" for ALL tiles simultaneously is a definitive sign that the `html5-app-deployer` CF task never ran (or was interrupted). The approuter serves tiles from its `localDir` (no HTML5 repo needed), so tiles appear correctly in the launchpad. Component loading is separate — it fetches `{url}/manifest.json` from `html5-apps-repo-rt`, which returns 404 if the deployer never completed. Root cause is almost always an interrupted `cf deploy` (CF session/token expiry mid-deploy leaves the HTML5 repo in its previous state). Fix: `cf deploy <mtar> --version-rule ALL` to force all modules including the `com.sap.application.content` deployer to re-run. Confirm success by checking deploy log for the line: "Deploying content module 'BridgeManagement-app-deployer' in target service 'BridgeManagement-html5-repo-host'...".
Applied: BMS v2.0.0 re-deploy with --version-rule ALL — 2026-05-22

[2026-05-22] [BTP / fioriSandboxConfig.json orphan inbounds] Learning: Any `ClientSideTargetResolution` inbound in `app/router/appconfig/fioriSandboxConfig.json` that points to a non-existent deployed HTML5 app URL (e.g. `/BridgeManagement/reports` when no `reports.zip` is in mta.yaml) causes "App could not be opened" when the intent is navigated to. Orphan intents can be created when: (a) a tile is removed but its inbound is not; (b) an app's mta.yaml module is deleted but the router config still references it; (c) a URL format change leaves old-format inbounds. Audit rule: for every inbound `url` in the BTP fioriSandboxConfig.json, verify a matching artifact name exists in the `BridgeManagement-app-deployer` build-parameters requires list in mta.yaml. Remove any inbound whose url has no matching deployed zip.
Applied: Removed orphan `Reports-display` inbound (pointed to `/BridgeManagement/reports`, no reports.zip in mta.yaml) — v2.0.1

[2026-05-22] [CAP / AdminService / Draft virtual fields] Learning: Any draft-enabled entity that has virtual fields computed in `after('READ', Entity)` MUST also have a corresponding `after('READ', Entity.drafts)` handler. Without it, FE4 OData V4 queries that combine active + draft records (the ListReport default filter `IsActiveEntity eq false OR SiblingEntity/IsActiveEntity eq null`) will flood the console with "Failed to drill-down into (ID=...,IsActiveEntity=false)/fieldName, invalid segment: fieldName" and cause page freezes or blank screens. Pattern:
- For entities with cheap virtual fields (enum mapping, date arithmetic): extract a shared helper function and register it on BOTH `Entity` and `Entity.drafts`.
- For entities with expensive virtual fields (DB COUNT queries, multi-table JOINs, BHI/BSI scoring): add a lightweight `after('READ', Entity.drafts)` that sets all virtual fields to safe defaults (`?? null` or `?? 0`) — NO DB queries, just null-coalescing.
Applied: srv/admin-service.js — after('READ', Bridges.drafts) for 9 virtual fields; _computeReviewCriticality on both Restrictions and Restrictions.drafts — v2.0.2

[2026-05-22] [FLP / innerAppRoute / sandbox] Learning: FLP sandbox bootstrap (`sandbox.js`) ignores `innerAppRoute` in `resolutionResult` for BOTH local dev AND BTP. Any tile that should navigate to a sub-route inside the admin-bridges app MUST encode the route directly in `targetURL` as `#Bridges-manage&/RouteTarget` — NOT as `#Maintenance-manage` with a separate `innerAppRoute`. The sub-domain `#Inspections-manage`, `#Maintenance-manage`, etc. intents in the ClientSideTargetResolution `inbounds` section are dead code in the sandbox and should not be relied on. Rule: always test tile click navigation as part of any tile config change — the tile appearing in the launchpad does not prove navigation works.
Applied: fioriSandboxConfig.json (local + BTP router) Work Orders tile: #Maintenance-manage → #Bridges-manage&/WorkOrdersList — v2.0.2

[2026-05-22] [UI5 / ComponentAnalyzer / Relative paths] Learning: The UI5 `generateComponentPreload` task's `ComponentAnalyzer` throws `Cannot read properties of undefined (reading 'toLowerCase')` and marks "(ignored)" when Component.js sap.ui.define array contains relative module paths (e.g. `"./fe-shims/NavServicePatch"`). The analyzer cannot resolve relative paths and crashes silently — the build still succeeds but the preload may be incomplete. Fix: either (a) remove unnecessary no-op stub dependencies from the sap.ui.define array, or (b) use the full AMD module path (`"BridgeManagement/adminbridges/fe-shims/NavServicePatch"`). Rule: never use relative `"./"` paths in the top-level `sap.ui.define([...])` dependency array of Component.js — always use the full dotted or slash-separated module path.
Source: BTP "Failed to load UI5 component" investigation — v2.0.4
Applied: app/admin-bridges/webapp/Component.js — removed ./fe-shims/NavServicePatch (was a no-op stub)

[2026-05-22] [FE4 / BridgeDetailExt.controller.js must be empty stub] Learning: `BridgeDetailExt.controller.js` (the file with `.controller.` in the name) must be an empty stub `sap.ui.define([], function(){"use strict"; return {};});`. FE4 resolves `sap.ui.controllerExtensions` entries to the dotted AMD path which maps to `BridgeDetailExt.js` (NO `.controller` suffix). The `BridgeDetailExt.controller.js` file is NOT loaded by FE4 at all — its only risk is polluting the preload or causing module registry conflicts if it contains real lifecycle code that duplicates `BridgeDetailExt.js`. The `onAfterRendering` sap.ui.require call for CustomAttributesInit belongs exclusively in `BridgeDetailExt.js` (the plain object). Having it in BOTH files means it fires twice per render cycle.
Source: BTP component load investigation — v2.0.4
Applied: app/admin-bridges/webapp/ext/controller/BridgeDetailExt.controller.js — neutralised to empty stub

[2026-05-22] [BTP / HTML5 App / Auth redirects misread as 404] Learning: A browser-based AI testing BTP URLs WITHOUT an active XSUAA session will receive HTTP 302 redirects to the login page when accessing HTML5 repo assets. The AI interprets this as "Component.js missing" or "file not found". Always verify BTP asset availability with a direct `curl -s -o /dev/null -w "HTTP %{http_code}"` call from the development machine (which can reach BTP without auth for HTML5 repo CDN paths). An HTTP 200 confirms the asset IS deployed correctly. An HTTP 302 would indicate an auth-gated path (check xs-app.json). An HTTP 404 would indicate the HTML5 deployer didn't run or the sap.app.id path is wrong.
Source: Browser AI false positive report — v2.0.4 post-deploy verification
Applied: Documented — verified all Component.js files return HTTP 200

[2026-05-22] [FLP / Tile Icons] Learning: Three icon values caused visible display problems in the SAP FLP sandbox: (1) `sap-icon://error` renders as a red ✕ symbol — not appropriate for a tile (use `sap-icon://quality-issue` for defect-related tiles); (2) `sap-icon://weight` is NOT a standard SAP icon — shows as empty/blank on tiles (use `sap-icon://simulate` for load/capacity tiles); (3) `sap-icon://overflow` renders as three dots ··· (the UI5 "more options" indicator) — not appropriate for a tile (use `sap-icon://area-chart` for hydraulic/scour assessment tiles).
Source: Browser AI tile audit — v2.0.5 fix
Applied: app/appconfig/fioriSandboxConfig.json + app/router/appconfig/fioriSandboxConfig.json

[2026-05-22] [BTP / HTML5Runtime_enabled / Trial Broker] Learning: The `HTML5Runtime_enabled: true` parameter in `mta.yaml` under the `BridgeManagement-app-deployer` → `BridgeManagement-html5-repo-host` requires block causes ALL HTML5 apps to return "App could not be opened" in BTP trial. The broker rejects this parameter with "html5_app_configuration/HTML5Runtime_enabled: Additional properties not allowed", leaving the HOST service in permanent `update failed` state. When the HOST service is in `update failed` state, the RT service cannot serve HTML5 content — all tile clicks fail regardless of how many times content is re-uploaded. Two-step fix is REQUIRED: (1) Remove `config: HTML5Runtime_enabled: true` from mta.yaml; (2) `cf delete-service BridgeManagement-html5-repo-host -f && cf create-service html5-apps-repo app-host BridgeManagement-html5-repo-host` to clear the failed state; (3) Redeploy the MTAR so the deployer uploads content to the fresh HOST service. Step (1) alone is NOT sufficient — the HOST service remains in `update failed` until deleted and recreated. WARNING: `curl` HTTP 200 responses from the BTP approuter are NOT proof of success — unauthenticated requests return XSUAA login redirect HTML (text/html, ~741 bytes) with HTTP 200. Always check `content-type` header to distinguish login redirect from real content.
Source: v2.0.5 → v2.0.6 → v2.0.6-redeploy investigation — all tiles "App could not be opened"
Applied: (1) mta.yaml — removed config.HTML5Runtime_enabled block; (2) cf delete-service + cf create-service; (3) cf deploy v2.0.6 MTAR --version-rule ALL → CODE 1002 upload confirmed

[2026-05-22] [BTP / Approuter / RT vs HOST binding] Learning: The approuter (v21.3.0+) must bind ONLY the html5-apps-repo **RT service** (plan: `app-runtime`, `sap.cloud.service: html5-apps-repo-rt`) in mta.yaml. Binding BOTH the RT service AND the HOST service (plan: `app-host`) simultaneously under the same `html5-apps-repo` label in VCAP_SERVICES causes the approuter to crash at startup with: `xs-app.json/routes/N: Format validation failed (A route requires access to html5-apps-repo-rt service but the service is not bound.)` This crash happens even though `cf env` shows both services bound. The HOST service is for the **deployer** (`com.sap.application.content` module type) only — add it to the deployer's `requires` block with `content-target: true`, never to the approuter's `requires`. In BTP trial, the RT service serves content from all HOST services in the subaccount without needing `app_host_id`. Correct diagnostic: OpenResty 503 (596 bytes) = RT service problem (likely HOST service in `update failed` state); XSUAA redirect HTML (737-741 bytes text/html) = approuter working, content accessible behind auth.
Source: v2.0.7 → v2.0.8 BTP deployment debugging session — 2026-05-22
Applied: mta.yaml — HOST service removed from approuter requires; approuter only requires html5-runtime (RT) + auth + destination + logging + srv-api

[2026-05-22] [UAT / FE4 draft-enabled entities / PATCH is 501] Learning: For draft-enabled entities in FE4/AdminService, direct PATCH to `Entity(ID=...,IsActiveEntity=true)` returns HTTP 501. To update an existing active record programmatically: (1) POST to `Entity(ID=...,IsActiveEntity=true)/AdminService.draftEdit` with `{"PreserveChanges":false}` → returns HTTP 201 with a new draft entity (different ID); (2) PATCH `Entity(ID=<draftId>,IsActiveEntity=false)` with the changes; (3) POST to `Entity(ID=<draftId>,IsActiveEntity=false)/AdminService.draftActivate`. The edit draft ID is different from the active entity ID. Applied for fixing CS-0001 and LR-0001 bridge_ID after UAT.
Source: UAT 2026-05-22 — fixing bridge_ID on existing BridgeConditionSurveys and BridgeLoadRatings records

[2026-05-22] [UAT / ProvisionTypes / SAP CodeList name vs description] Learning: `ProvisionTypes` (and all SAP CodeList entities) have TWO text fields: (1) `name` — the SAP standard display field used by some FE4 value-help renderings; (2) `description` — our custom field. Always populate BOTH in seed CSVs. Pattern: add `name` column with the same value as `description` as a minimum. The `descr` field (SAP short description) is optional but should also be populated for completeness. Fix: add `name` column to `db/data/bridge.management-ProvisionTypes.csv`.
Source: UAT 2026-05-22 — ProvisionTypes value-help showing blank name in FE4 dropdown

[2026-05-22] [UAT / Verification pattern for bridge navigation in list reports] Learning: When a ListReport shows blank values in a navigation-path column (e.g. `bridge.bridgeName`), verify with OData before concluding it is an annotation bug: `fetch('/odata/v4/admin/EntitySet?$expand=bridge($select=bridgeName)&$select=entityRef,bridge_ID')`. If `bridge.bridgeName` is populated in the OData response, the annotation is correct and the issue is either a data gap (null `bridge_ID`) or a UI rendering timing issue. Fix the data first; only fix annotations if OData also returns null. Applied: confirmed Work Orders already show "Lennox Bridge" correctly — no annotation fix needed.
Source: UAT 2026-05-22 — P2-001 false positive investigation

[2026-05-23] [BTP / Express Router / requireScope ordering] Learning: `requireScope('admin')` applied to the ENTIRE `/system/api` router blocks ALL users except admins — including the GET `/system/api/features` endpoint that every app polls on startup via `initFeatureFlags()`. Result: non-admin BMS users see "Error: Forbidden" dialog immediately on app load. Fix: broaden the router-level scope to include ALL BMS role scopes: `requireScope('admin', 'manage', 'operate', 'inspect', 'view', 'executive_view', 'certify', 'config_manager')`. Then add a FINE-GRAINED scope guard INSIDE only the mutating endpoints (PATCH /features) that actually require elevated access. Never use a narrow router-level scope when the router contains mixed read/write endpoints serving different user roles.
Source: BTP v2.0.13 — "Error: Forbidden" dialog on all non-admin user logins

[2026-05-23] [BTP / Express Router / featureFlagsRouter orphan] Learning: When a sub-router (featureFlagsRouter) is removed from one mount point (`app.use('/system/api/features', ...)` in server.js), it must be re-mounted inside the parent router (`router.use('/features', featureFlagsRouter)` in system-router.js). Forgetting this step silently discards all routes defined in the sub-router — Express processes routes in registration order and never reaches an unmounted router. The symptom is HTTP 404 on `/system/api/features` even though the file is required. Pattern: whenever you change where a router is mounted, immediately grep for all `router.use/get/post/patch` calls in that router file and verify they're reachable from the new mount point.
Source: BTP v2.0.13 — featureFlagsRouter unmounted after server.js cleanup

[2026-05-23] [BTP / CSP / SAP Logo] Learning: The SAP Fiori shell renders the SAP logo from the UI5 CDN (`https://sapui5.hana.ondemand.com/...`). If `img-src` in the Content-Security-Policy header (in `app/router/xs-app.json` → `responseHeaders` array) does not include `https://ui5.sap.com` and `https://sapui5.hana.ondemand.com`, the browser blocks the logo image and shows the text "SAP Logo" instead. This is a SILENT failure — no console error appears unless DevTools is open. Always verify that BOTH CDN origins are in `img-src` after any xs-app.json CSP change. The server.js Helmet CSP (`srv/server.js`) is a SEPARATE policy that applies to `/odata/*` responses — updating it alone is NOT sufficient for the approuter.
Source: BTP v2.0.13 — "SAP Logo" text shown instead of SAP logo image

[2026-05-23] [BTP / package.json vs mta.yaml version] Learning: `mta.yaml` version and `package.json` version are independent. `npm start` reports the `package.json` version (e.g. `> BridgeManagementV2@2.0.11 start`), which is also what `GET /health` returns as `version`. `mta.yaml` version controls the MTA descriptor and CF deploy version tracking. When bumping for a release, ALWAYS update BOTH files: (1) `mta.yaml` version + APP_VERSION property; (2) `package.json` version. A mismatch causes confusion — the health endpoint reports the wrong version while CF deploys show the correct mta.yaml version.
Source: BTP v2.0.13 deploy — health endpoint showing v2.0.11 while mta.yaml said v2.0.13

[2026-05-23] [BTP / AdminService / No top-level @requires is intentional] Learning: AdminService in `srv/admin-service.cds` has NO top-level `@requires` annotation — this is intentional. CAP's default without `@requires` is "authentication required" (any valid token). Individual entity-level `@restrict` annotations provide the fine-grained access control per entity. The entities without `@restrict` (lookup tables like States, Regions, StructureTypes, etc.) are intentionally open to all authenticated users — lookup data must be readable for value-helps to work for ALL user roles. Do NOT add a top-level `@requires` without reviewing every entity individually — lookup entities would then require explicit `@restrict` to remain readable, and the value-help pickers throughout the app would break for non-admin users.
Source: BTP v2.0.14 code audit
